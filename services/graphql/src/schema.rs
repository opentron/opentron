    ...
    async fn node_info(&self, ctx: &Context<'_>) -> NodeInfo {
        ...
        NodeInfo {
            name: format!(
                "OpenTron/v{}/{}/{}({}-{})",
                CODE_VERSION,
                system.host_name().unwrap_or("localhost".into()),
                system.long_os_version().unwrap_or("unknown-os-ver".into()),
                system.name().unwrap_or("unknown-os".into()),
                system.kernel_version().unwrap_or("unknown-ver".into()),
            ),
